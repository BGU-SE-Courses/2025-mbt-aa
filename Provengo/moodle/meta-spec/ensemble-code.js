...
const GOALS = [
    Event('Start-enterToForumDiscussionAndDeleteDiscussion'),
    Event('Start-enterToForumDiscussionAndSubscribe')
];
...
function rankByMetGoals( ensemble ) {
    const unreachedGoals = [];
    for ( let idx0=0; idx0<GOALS.length; idx0++ ) // we want all the combinations
        for (let idx1 = 0; idx1 < GOALS.length; idx1++)
            if (idx0 !== idx1)
                unreachedGoals.push([GOALS[idx0], GOALS[idx1]]);
            
        
    let unreachedGoalsInitLen = unreachedGoals.length
    for (let testIdx = 0; testIdx < ensemble.length; testIdx++) {
        let test = ensemble[testIdx];
        for (let ugIdx = unreachedGoals.length - 1; ugIdx >= 0; ugIdx--) { // swiched between the fors
            let c = 0;
            let unreachedGoal = unreachedGoals[ugIdx];
            for (let eventIdx = 0; eventIdx < test.length; eventIdx++) {
                let event = test[eventIdx];
                if (unreachedGoal[0].contains(event))
                    c = 1
                else if (unreachedGoal[1].contains(event) && c === 1)
                    unreachedGoals.splice(ugIdx, 1);
            }

        }
    }
    return unreachedGoalsInitLen - unreachedGoals.length;
}
 ...
 */
 function rankingFunction(ensemble) {
    ...
    const metGoalsCount = rankByMetGoals(ensemble);
    // What percentage of the goals did `ensemble` cover?
    const metGoalsPercent = metGoalsCount/(GOALS.length * (GOALS.length-1));
